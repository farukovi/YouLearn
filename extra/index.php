...
<div class="container">
	<form  method="post" action="<?= htmlspecialchars($_SERVER['PHP_SELF']); ?>" style="background: rgba(0, 179, 0,.9);">
		<div class="jumbotron"style="background: rgba(0, 179, 0,.9);">
			...
			<button type="submit" formaction="YouLearn.html" class="btn btn-success">Back</button>
			<input type="Submit" class="btn"  name="login" value="Login"></center>
		</div>
	</form>
</div>
...
if(isset($_POST['login']))
{
	...
	if(mysqli_num_rows($result)>0)
	{
		...
		else if($type=="Child")
		{
			...
			{
				$rows = mysqli_fetch_array($res);
				$_SESSION['ChildId'] = $rows['Name'];
				$sql = "SELECT COUNT(Id) FROM exam_detaiils";
				$result = mysqli_query($con,$sql);
				if(mysqli_num_rows($result)>0){
					$row = mysqli_fetch_array($result);
					$num = $row['COUNT(Id)'];
				...
				<?php
						...
						?>
							<script>
							window.alert('Exam, Welcome Student!');
							window.location.href='Child/Exam.php';
							</script>
				<?php
					}
				}
